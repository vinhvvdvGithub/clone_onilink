    ...
    private function handle_image_upload($uploaded_image, $upload_folder, $size_limit) {
        ...
        if($image) {
            ...
            if(!is_writable(UPLOADS_PATH . $upload_folder)) {
                Response::json(sprintf(language()->global->error_message->directory_not_writable, UPLOADS_PATH . $upload_folder), 'error');
            }
            ...
            if($_FILES['image']['size'] > $size_limit * 1000000) {
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                try {
                    ...
                    if(!empty($uploaded_image)) {
                        $s3->deleteObject([
                            'Bucket' => settings()->offload->storage_name,
                            'Key' => 'uploads/block_thumbnail_images/' . $uploaded_image,
                        ]);
                    }
                    ...
                    $result = $s3->putObject([
                        'Bucket' => settings()->offload->storage_name,
                        'Key' => 'uploads/block_thumbnail_images/' . $image_new_name,
            ...
            /* Local uploading */
                ...
                /* Delete current file */
                if(!empty($uploaded_image) && file_exists(UPLOADS_PATH . $upload_folder . $uploaded_image)) {
                    unlink(UPLOADS_PATH . $upload_folder . $uploaded_image);
                }

                /* Upload the original */
                move_uploaded_file($image_file_temp, UPLOADS_PATH . $upload_folder . $image_new_name);
            }
