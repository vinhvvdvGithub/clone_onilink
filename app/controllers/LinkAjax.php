...
class LinkAjax extends Controller {
    ...
    private function update_biolink() {
        ...
        if($image && !$image_delete) {
            ...
            if(!is_writable(UPLOADS_PATH . 'avatars/')) {
                Response::json(sprintf(language()->global->error_message->directory_not_writable, UPLOADS_PATH . 'avatars/'), 'error');
            }
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                try {
                    ...
                    $s3->deleteObject([
                        ...
                        'Key' => 'uploads/avatars/' . $link->settings->image,
                    ]);

                    /* Upload image */
                    $result = $s3->putObject([
                        'Bucket' => settings()->offload->storage_name,
                        'Key' => 'uploads/avatars/' . $image_new_name,
        ...
        if($seo_image) {
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                try {
                    ...
                    $result = $s3->putObject([
                        'Bucket' => settings()->offload->storage_name,
                        'Key' => 'uploads/block_images/' . $seo_image_new_name,
                        'ContentType' => mime_content_type($seo_image_file_temp),
                        'SourceFile' => $seo_image_file_temp,
                        'ACL' => 'public-read'
                    ]);
                } catch (\Exception $exception) {
                    Response::json($exception->getMessage(), 'error');
        ...
        $seo_image_url = $db_seo_image ? UPLOADS_FULL_URL . 'block_images/' . $db_seo_image : null;
