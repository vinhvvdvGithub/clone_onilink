...
<div class="card mb-5">
    <div class="card-body">
        <h2 class="h4"><i class="<?= language()->link->biolink->{$key}->icon ?> fa-xs text-muted"></i> <?= language()->link->biolink->{$key}->name ?></h2>
        <div class="d-flex flex-column flex-xl-row">
            <div class="mb-2 mb-xl-0 mr-4">
                <span class="font-weight-bold"><?= nr($data->total[$key]) ?></span> <?= language()->link->biolink->{$key}->name ?>
            </div>
        </div>
...
<script>
    ...
    new Chart(<?= $key ?>_chart, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: <?= json_encode(language()->link->biolink->{$key}->name) ?>,
